                ...
                responseFuture.thenAcceptAsync(response -> {
                    if (response.isSuccess()) {
                        ...
                        if (finalItemType == null) {
                            inventory.clear();
                            CommandSenderHelper.send(api.getScheduler(), sender, ChatColor.GREEN + "Cleared " + userName + "'s enderchest.");
                        } else {
                            if (finalMaxCount == -1) {
                                finalItemType.removeAllFrom(inventory);
                                CommandSenderHelper.send(api.getScheduler(), sender, ChatColor.GREEN + "Removed all " + finalItemType + " from " + userName + "'s enderchest.");
                            } else {
                                int removed = finalItemType.removeAtMostFrom(inventory, finalMaxCount);
                                CommandSenderHelper.send(api.getScheduler(), sender,  ChatColor.GREEN + "Removed " + removed + " " + finalItemType + " from " + userName + "'s enderchest.");
                            }
                        }
                        ...
                        if (reason instanceof TargetDoesNotExist) {
                            ...
                            CommandSenderHelper.send(api.getScheduler(), sender, ChatColor.RED + "An internal fault occurred when trying to load " + implementationFault.getTarget() + "'s enderchest.");
                        } else if (reason instanceof OfflineSupportDisabled) {
                            CommandSenderHelper.send(api.getScheduler(), sender, ChatColor.RED + "Spectating offline players' enderchests is disabled.");
                        } else {
                            CommandSenderHelper.send(api.getScheduler(), sender, ChatColor.RED + "Cannot clear from " + inputPlayer + "'s enderchest for an unknown reason.");
                        }
                    }
